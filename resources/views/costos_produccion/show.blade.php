                ...
                <table class="w-full mb-6 border border-black rounded">
                    ...
                    <tbody>
                        <tr class="border border-black odd:bg-white even:bg-gray-200">
                            ...
                            <td class="border px-1">
                                {{ number_format($totalIndirectas, 2, ',', '.') }}
                ...
                <div class="card">
                    <div class="card-body">
                        <table class="table">
                            ...
                            <tbody>
                                @foreach ($costosProduccion as $key => $costos)
                                    @php
                                        // Separar los elementos agrupados por operario, articulo y servicio
                                        list($operario, $articulo, $servicio) = explode('-', $key);
                                    @endphp
                                    <tr>
                                        <td>{{ $operario }}</td>
                                        <td>{{ $servicio }}</td>
                                        <td>{{ $costos['horas'] }}</td>
                                        <td>{{ number_format($costos['mano_obra_directa'], 2) }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                            <tfoot>
                ...
                <table class="table">
                    ...
                    <tbody>
                        @foreach ($materiasPrimasDirectasSubtotales as $materiaPrimaDirecta)
                            <tr>
                                <td>{{ $materiaPrimaDirecta->codigo }}</td>
                                <td>{{ $materiaPrimaDirecta->descripcion }}</td>
                                <td>{{ $materiaPrimaDirecta->pivot->cantidad }}</td>
                                <td>{{ number_format($materiaPrimaDirecta->precio_unit, 2, ',', '.') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                    <tfoot>
                ...
                <table class="table">
                    ...
                    <tbody>
                        @foreach ($materiasPrimasIndirectasSubtotales as $materiaPrimaIndirecta)
                            <tr>
                                <td>{{ $materiaPrimaIndirecta->codigo }}</td>
                                <td>{{ $materiaPrimaIndirecta->descripcion }}</td>
                                <td>{{ $materiaPrimaIndirecta->pivot->cantidad }}</td>
                                <td>{{ number_format($materiaPrimaIndirecta->precio_unit, 2, ',', '.') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                    <tfoot>
